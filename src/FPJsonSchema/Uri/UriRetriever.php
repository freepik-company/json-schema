...
use FPJsonSchema\Uri\Retrievers\UriRetrieverInterface;
use FPJsonSchema\UriRetrieverInterface as BaseUriRetrieverInterface;
...
class UriRetriever implements BaseUriRetrieverInterface
{
     ...
     *
     * @param object $FPJsonSchema JSON Schema contents
     ...
     */
    public function resolvePointer($FPJsonSchema, $uri)
    {
        ...
        while ($path) {
            ...
            if (!empty($pathElement)) {
                ...
                if (!empty($FPJsonSchema->$pathElement)) {
                    $FPJsonSchema = $FPJsonSchema->$pathElement;
                } else {
    ...
    public function retrieve($uri, $baseUri = null, $translate = true)
    {
        ...
        $FPJsonSchema = $this->loadSchema($fetchUri);

        // Use the JSON pointer if specified
        $FPJsonSchema = $this->resolvePointer($FPJsonSchema, $resolvedUri);

        if ($FPJsonSchema instanceof \stdClass) {
            $FPJsonSchema->id = $resolvedUri;
        }
    ...
    protected function loadSchema($fetchUri)
    {
        ...
        $FPJsonSchema = json_decode($contents);

        if (JSON_ERROR_NONE < $error = json_last_error()) {
            throw new JsonDecodingException($error);
        }

        $this->schemaCache[$fetchUri] = $FPJsonSchema;
